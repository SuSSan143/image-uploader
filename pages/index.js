...
export default function Home() {
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(uploadedImage[SMALL].url).then(() => {
      alert(`Coppied ${uploadedImage[SMALL].url} to clipboard`);
    });
  };

  console.log(uploadedImage)

  if (isImageShowing && uploadedImage[SMALL].hasOwnProperty("url")) {
    return (
      <>
        <Head>
          <title>My Image</title>
          <meta name="description" content="Generated by create next app" />
          <link rel="icon" href={uploadedImage[SMALL].url} />
        </Head>
        <ImageContainer>
          <StyledImage
            src={uploadedImage[SMALL].url}
            height={uploadedImage[SMALL].height}
            width={uploadedImage[SMALL].width}
            alt="uploadedImage"
          />
          <LinkContainer>
            <LinkText>{`${uploadedImage[SMALL].url}`}</LinkText>
